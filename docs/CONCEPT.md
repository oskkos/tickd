...
of this document — so the design reads cleanly without losing the history.
...
**Four surfaces, reached from a bottom tab bar:** the logging screen (which contains the venue picker
and the end-of-session summary), a **session list** with a per-session detail view, the flash-rate
chart, and **settings** — which is where export and import live. `DESIGN.md` §6 has listed these five
since it was written; this list named neither the session list nor settings, and a screen that exists in
only one of the two documents is exactly the drift the CONCEPT/DESIGN split is meant to prevent. Fixed
here, since CONCEPT owns *what*.

The session detail is not an extra: it is the only read path for the optional fields the tick sheet
collects (`notes`, `rating`, `grade_opinion`, `angle`, `holds`, `length_m`). Without it those are
write-only once the session closes, and a logbook you cannot read back does not meet the exit criterion
below. It is also the only **repair** path once a session closes: a go's grade, protection and outcome
are correctable from there, per go and never across a discipline (D23).
...
**A tick's paired unions are corrected whole; everything else is patched.** `grade`,
`discipline`+`protection` and the outcome are replaced as complete values, because a partial of a
discriminated union is unsound — half of one member and half of another is a row that is counted
inconsistently rather than rejected. The annotation fields sit outside those unions and are patched
individually. A correction preserves `created_at`, `date_local` and `tz_offset`: a climb belongs to the
local day it was climbed, so editing a go is not the same operation as logging it again (D23).
...
**One file per decision, in [`decision-log/`](decision-log/).** They lived here as `###` sections until
the log outgrew the document it was appended to — twenty-three entries and roughly 750 lines, over a
third of a file whose job is to describe the *current* design. Splitting them keeps the body readable
and gives each decision a citable file, and `decision-log/README.md` owns the structure they share.

**Cite them by number, as everything already does.** `D17` is the key: it appears in code comments,
in the capability specs and in `CLAUDE.md`, so the numbers are stable and a decision is never
renumbered. Filenames carry the number and the subject, so the folder reads as a table of contents.

| | Decision |
|---|---|
| **D1** | [Indoor-first; outdoor conceded](decision-log/01-indoor-first-outdoor-conceded.md) |
| **D2** | [No route entity; `route` merged into `tick`](decision-log/02-no-route-entity.md) |
| **D3** | [Route lifecycle fields dropped entirely](decision-log/03-route-lifecycle-fields-dropped.md) |
| **D4** | [Corrections: plain `DELETE`, not tombstones](decision-log/04-corrections-are-plain-deletes.md) |
| **D5** | [Grades: what's in, what's out](decision-log/05-grades-whats-in-whats-out.md) |
| **D6** | [Style: two fields, not one flat enum](decision-log/06-style-is-two-fields-not-one-enum.md) |
| **D7** | [Phase 0 backup dropped, and with it the Phase 1 claim path](decision-log/07-phase-0-backup-dropped.md) |
| **D8** | [Hosting: the "no scale-to-zero" requirement was wrong](decision-log/08-scale-to-zero-requirement-was-wrong.md) |
| **D9** | [Backend: Kotlin over Node, twice](decision-log/09-kotlin-backend-over-node.md) |
| **D10** | [Quarkus considered, declined](decision-log/10-quarkus-considered-declined.md) |
| **D11** | [Not a portfolio artifact](decision-log/11-not-a-portfolio-artifact.md) |
| **D12** | [Smaller merges and simplifications](decision-log/12-smaller-merges-and-simplifications.md) |
| **D13** | [Estimates, as revised](decision-log/13-estimates-as-revised.md) |
| **D14** | [Flash rate divides by first encounters, not by sends](decision-log/14-flash-rate-divides-by-first-encounters.md) |
| **D15** | [Play Store is a channel, not a rewrite](decision-log/15-play-store-is-a-channel-not-a-rewrite.md) |
| **D16** | [Phase 0 hosting written down; the origin change is accepted, not avoided](decision-log/16-phase-0-hosting-and-the-origin-change.md) |
| **D17** | [A scale is a notation, not a discipline](decision-log/17-a-scale-is-a-notation-not-a-discipline.md) |
| **D18** | [`second_go` and `attempts` dropped; a send is flashed or it is not](decision-log/18-second-go-and-attempts-dropped.md) |
| **D19** | [`tick.venue_id` dropped; a tick reaches its venue through its session](decision-log/19-tick-venue-id-dropped.md) |
| **D20** | [A tick is one go, so `send_style` is derived rather than stored](decision-log/20-a-tick-is-one-go-send-style-derived.md) |
