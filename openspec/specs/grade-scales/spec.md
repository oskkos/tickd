...
namespace, incomparable values get ranked against each other and every metric is silently wrong
(`CONCEPT.md` §7.3, D5).

**That guarantee is about notations, not disciplines.** A scale does not imply a discipline — French
serves rope everywhere and boulders at venues that grade them that way — so keeping boulders apart from
routes belongs to the consuming layer, via `discipline` and `protection` on the tick. This capability
separates scales; it cannot separate disciplines, and nothing here should be read as doing so
(`CONCEPT.md` §4.2, D17).
