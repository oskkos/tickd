...
The detail is also the only **repair** path once a session closes. A go's grade, its protection and its
outcome are correctable there, through writes that replace a whole paired union rather than patching part
of one. This paragraph used to say the opposite — that the sheet reached the annotation fields only, and
that a mis-tapped `prior_experience` was permanently wrong — which was true when the capability shipped
and is what D23 closed.

What it deliberately does not do: no analytic of any kind (flash rate has its own screen, volume metrics
are Phase 1), no deletion (undo stays session-scoped, for mis-taps at the wall), and no correction across
a **discipline** — the scale travels with it, so a go logged as a boulder when it was a roped route stays
wrong, which is stated rather than implied away (D17, D23).
