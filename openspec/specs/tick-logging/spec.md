...
The logging screen: the two taps that produce a tick, the session that gives it a venue, and the optional
detail that hangs off it afterwards. `CONCEPT.md` §3 names this the wedge — *"tap grade → tap style →
logged"* — so the tap count is a requirement here rather than a target, in every case and not only when
