...
The fourth Phase 0 surface, and the one carrying durability. `CONCEPT.md` §7.6 accepts evictable storage
on the grounds that manual export and import exist as the bridge — this is where they exist. It owns the
file the logbook is written to, the three ways an import is refused, the deletion that shares its reach,
and the two device preferences.

The refusals are the substance. A schema-marker mismatch is refused rather than upgraded, because
upgrading would be a Dexie migration by another name (D7). A file whose digest no longer matches its
payload is refused too, because the marker cannot tell a file that came from this schema from one that is
*true* to it — JSON expresses rows the type system forbids, and import is the only write path with no
compiler behind it (D24).

It also reports something no other surface can: whether the browser is protecting the logbook from
eviction, and what to do when it is not. That was reaching `console.info` and nowhere else.

Preferences live in `localStorage` rather than a fourth table, so the schema marker does not move and an
export stays a logbook rather than becoming a device image.
