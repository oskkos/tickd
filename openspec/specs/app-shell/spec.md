...
top-level surface and no placeholders. In Phase 0 that is `Log`, `Sessions`, `Flash` and `Settings` — the
four surfaces `CONCEPT.md` §"Phase 0" names, so the bar is complete and no tab remains deferred. The rule
outlives that completeness: a tab appears when its surface does, never before.
