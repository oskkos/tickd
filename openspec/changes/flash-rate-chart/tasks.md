...
- [x] 6.1 Run `just check` and confirm codegen, format, typecheck, lint and the full suite pass
- [x] 6.2 Drive the surface in a real browser at 412×600 with the tab bar present: confirm a bar's rendered width matches its rate, that the reference rule lands where the fills are read against it, and that the suppressed and real-zero rows are distinguishable at a glance
